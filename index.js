...
function customAssign(...objects) {
  ...
  let rslt = Object.assign({}, ...objects);
  // Використання методу `assign` для об'єднання об'єктів
  for (const key in rslt) {
    if (typeof rslt[key] === "number") {
      rslt[key] = String(rslt[key]).replace(" ", "_");
    }
  }
...
function customEntries(obj) {
  ...
  let result = Object.entries(obj); // Використання методу `entries` для отримання масиву з усіма властивостями та значеннями
  return result; // Повернення масиву властивостей та значень
}
...
function customObjectFromEntries(entries) {
  ...
  let result1 = entries.map(([key, value]) => {
    if (typeof key === "number") {
      value = String(key).replace(" ", "_");
    }
    return [key, value];
  ...
  let result2 = Object.fromEntries(result1); // Використання методу `fromEntries` для створення об'єкта з масиву записів
  return result2; // Повернення створеного об'єкта
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  let obj = Object.assign(obj1, obj2); // Використовуємо метод Object.assign() для додавання властивостей obj2 до obj1
  let result = Object.freeze(obj); // Використовуємо метод Object.freeze() для "заморожування" об'єкта, запобігаючи подальшим змінам
  if (Object.isFrozen(obj) !== "false") {
    return obj;
  }
  return Error; // Перевіряємо, чи об'єкт "заморожено"
...
function keysAndValues(obj) {
  ...
  let rslt1 = Object.keys(obj); // Використовуємо метод Object.keys() для отримання масиву ключів об'єкта
  let rslt2 = Object.values(obj); // Використовуємо метод Object.values() для отримання масиву значень об'єкта
  return [rslt1, rslt2]; // Повертаємо масив з масивами ключів та значень
}
...
function getObjectValuesSum(obj) {
  ...
  const value = Object.values(obj); // Отримуємо всі значення об'єкта
  const sum = value.reduce((acc, cur) => acc + cur, 0); // Обчислюємо суму значень
...
//   let obj = {}; // Створюємо пустий об'єкт який записуємо в змінну
//   for (let i = 0; (i = arr.length); i++) {
//     const [key, value] = arr[i];
//     if (obj.hasOwnProperty(key)) {
//       console.log(`Y масиві є дубльований ключ: ${key}`);
//     }
